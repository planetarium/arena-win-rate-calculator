  ...
  const Controller = () => (
    <div>
      ...
      <div className="flex justify-center gap-4 select-none">
        ...
        <button
          className={`flex mb-4 items-center justify-center w-8 h-8 rounded-full ${!hasMoreData ? "" : "bg-neutral-50 text-neutral-950"}`}
          onClick={() => handlePageChange(currentPage + 1)}
        >
          »
        </button>
    ...
    <div className="flex flex-col flex-1">
      ...
      <div className="flex-1 flex flex-col gap-8 py-4 mb-8">
        <div className="flex-1">
          ...
          {arenaInfos.map((arena, index) => (
            <div
              className={`px-8 py-4 border-b border-neutral-700 flex gap-3 items-center ${arena.avatar.code === avatarAddress ? "bg-neutral-800" : ""}`}
            >
              <div className="flex-shrink-0 flex-grow-0 w-12">{arena.rank}</div>
              <div className="flex-shrink-0 flex-1">
                <p>
                  <span className="font-semibold">{arena.avatar.name}</span>
              ...
              <div className="flex-shrink-0 flex-grow-0 w-16 text-center">
                {arena.winRate === undefined && (
                  <button
                    ...
                    onClick={() => handleWinRateClick(index)}
                  >
                ...
                {typeof arena.winRate === "number" && (
                  <div>{arena.winRate}%</div>
                )}
              </div>
            </div>
          ))}
